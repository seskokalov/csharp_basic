    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            */
            ...
            List<Song> songsThatStartWithLetterA = Songs.Where(x => x.Name.ToLower().StartsWith("a")).ToList();
            Console.WriteLine("Songs start with the letter 'a' (case insensitive)");
            foreach (Song item in songsThatStartWithLetterA)
            {
                Console.WriteLine(item.Name);
            }
            Console.WriteLine("-------------------------------------------------------------------------------");

            List<Artist> artistThatEndWithA = Artists.Where(x => x.FullName.ToLower().EndsWith("a")).ToList();
            Console.WriteLine("artists end with letter 'a' (case insensitive)");
            foreach (Artist item in artistThatEndWithA)
            {
                Console.WriteLine(item.FullName);
            }
            Console.WriteLine("-------------------------------------------------------------------------------");

            Song longestSong = Songs.LastOrDefault(x => x.Duration == Songs.Select(y => y.Duration).Max());
            Console.WriteLine($"The longest song is {longestSong.Name} with a duration of {longestSong.Duration}");
            Console.WriteLine("-------------------------------------------------------------------------------");

            int totalDurationOfSongs = Songs.Select(x => x.Duration).Sum();
            Console.WriteLine($"The total duration of the songs is {totalDurationOfSongs}");
            Console.WriteLine("-------------------------------------------------------------------------------");

            var albumsWithSongsLongerThan300 = Songs.Where(x => x.Duration > 300).Select(x => x.AlbumId)
                                                    .GroupBy(s => s)
                                                    .Select(g => new { AlbumCounter = g.Key, Count = g.Count () });
                                    
            Console.WriteLine($"The number of albums that have song longer than 300 seconds is: {albumsWithSongsLongerThan300.Count()}");
            Console.WriteLine("-------------------------------------------------------------------------------");

            List<string> namesOfArtistsPopRock = Artists.Where(x => x.Albums.Where(s => s.Genre == Genre.PopRock).ToList().Count >= 1).Select(x => x.FullName).ToList();
            namesOfArtistsPopRock.ForEach(artist => Console.Write($"--{artist}--"));
            Console.WriteLine("-------------------------------------------------------------------------------");

            Album albumWithHighestAverageOfSongDuration = Albums.OrderByDescending(x => x.Songs.Select(y => y.Duration).Average()).FirstOrDefault();
            Console.WriteLine($"Album with highest song duration average is {albumWithHighestAverageOfSongDuration.Name}");
            Console.WriteLine("-------------------------------------------------------------------------------");

            int charactersOfLongestSong = Songs.OrderByDescending(x => x.Duration).LastOrDefault().Name.Length;
            Console.WriteLine($"Shortset duration song has {charactersOfLongestSong} characters");
            Console.WriteLine("-------------------------------------------------------------------------------");

            var nameAndGenreOfAlbumWithMostSongs = Albums.OrderByDescending(x => x.Songs.Count).LastOrDefault();
            Console.WriteLine($"Album with most songs is {nameAndGenreOfAlbumWithMostSongs.Name}, number of songs {nameAndGenreOfAlbumWithMostSongs.Songs.Count} with genre {nameAndGenreOfAlbumWithMostSongs.Genre}");
            Console.WriteLine("-------------------------------------------------------------------------------");

            //var artistWithMostSongs = Artists.OrderBy(x => x.Id == Albums.Where(y => y.Id == Songs.Where(z => z.AlbumId)).
            //                                    .GroupBy(s => s)
            //                                    .Select(g => new { SongCounter = g.key, Count = g.Count() }));
